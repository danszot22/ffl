  ...
  const loadTableWithData = useCallback(async () => {
    const filterColumnsChanged = (arr1, arr2) => {
      return (
        ...
        arr1.some(
          (column, index) =>
            column.id !== arr2[index].id || column.value !== arr2[index].value
        )
      );
    };
    ...
    const sortColumnsChanged = (arr1, arr2) => {
      return (
        ...
        arr1.some(
          (column, index) =>
            column.id !== arr2[index].id || column.desc !== arr2[index].desc
        )
      );
    };

    const getPreviouslyFetchedData = ({ pageIndex, pageSize }) => {
      return fetchedData.slice(
        pageIndex * pageSize,
        (pageIndex + 1) * pageSize
      );
    };

    const filtersChanged = filterColumnsChanged(
      columnFilters,
      prevColumnFilters
    );
    ...
    try {
      if (
        sortingChanged ||
        filtersChanged ||
        fetchedData.length <= pagination.pageIndex * pagination.pageSize
      ) {
        const results = await loader(
          { pagination, columnFilters, globalFilter, endCursor, sorting },
          !(filtersChanged || sortingChanged)
        );
        ...
        if (results) {
          if (results.items.length > 0) {
            ...
            if (filtersChanged || sortingChanged) setFetchedData(items);
            else
              setFetchedData(
                fetchedData.length ? fetchedData.concat(items) : items
              );
          }
          if (!results.hasNextPage)
            setRowCount(fetchedData.length + results.items.length);
          else setRowCount((pagination.pageIndex + 2) * pagination.pageSize);
        }
      } else {
