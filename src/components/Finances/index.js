  ...
  useEffect(() => {
    const fetchFinances = async () => {
      try {
        ...
        setTeams(
          mapTeamFinances(responseFinances, responsePrizes, responseFees)
        );
      } catch (error) {
        console.error(error);
        return;
  ...
  const columns = [
    {
      ...
      editable: false,
      renderCell: (params) => <TeamLink team={params.row} />,
    },
    {
      field: "EntryFee",
      width: isBelowMedium ? 75 : 150,
    ...
    },
    {
      ...
      valueGetter: (params) => {
        return `${
          params.row.EntryFee +
          params.row.TransactionFees -
          params.row.FeesPaid -
          (params.row.Winnings - params.row.WinningsPaid)
        }`;
      },
    },
  ];
