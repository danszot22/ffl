...
function LineupList({ league, team, user }) {
  ...
  useEffect(() => {
    const fetchLineups = async (leagueId, lineupWeek) => {
      ...
      setLineups(
        mapToLineupList(lineupResponse, scoring, nflGameResponse, team?.TeamId)
      );
      setIsLoading(false);
    };
  ...
  useEffect(() => {
    ...
    if (nflWeekState?.lineupWeek) setWeek(nflWeekState.lineupWeek);
  }, [nflWeekState]);
          ...
          >
            {lineups.map((lineup, index) => (
              <MenuItem
                ...
                value={index}
              >{`${lineup.team.OwnerName}`}</MenuItem>
            ))}
          </Select>
        </FormControl>
        <FormControl fullWidth>
          ...
          >
            ...
            >
              {lineups.map((lineup, index) => (
                <Tab
                  key={lineup.team.TeamId}
                  label={formatFantasyTeamName(lineup.team)}
                  {...a11yProps(index)}
                />
              ))}
            </Tabs>
                        ...
                        >
                          ...
                          {!selectedTeam?.complete ? (
                            <Typography variant="caption">
                              Projected:{" "}
                              {selectedTeam?.projectedScore.toFixed(1)}{" "}
                            </Typography>
                          ) : null}
                          {!selectedTeam?.complete && team?.score > 0 ? (
                            <Typography variant="caption">
                              Actual: {selectedTeam?.score.toFixed(1)}{" "}
                            </Typography>
                          ) : null}
                          {selectedTeam?.complete ? (
                            <Typography variant="caption">
                              Finished: {selectedTeam?.score.toFixed(1)}{" "}
                            </Typography>
                          ) : null}
                        </Box>
                      </TableCell>
