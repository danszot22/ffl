...
import { teamLoader, leaguePlayersLoader, teamScheduleLoader, teamTransactionsLoader, teamWaiverRequestsLoader } from "../../api/graphql";
...
function Team({ league, team, user }) {
    ...
    useEffect(() => {
        if (league?.LeagueId && teamDetails?.LeagueId && league?.LeagueId !== teamDetails?.LeagueId) {
            if (!team.TeamId)
                navigate('/');
            else
                navigate('/Team');
    ...
    useEffect(() => {
        const fetchTeam = async (teamId) => {
            ...
            setRoster(mapToRosterList(response, teamId).find(rosterPlayer => rosterPlayer?.team.TeamId === teamId));
        }
    ...
    function handleGoToTeamSettings() {
        navigate(`/TeamSettings/${teamDetails?.LeagueId}/${teamDetails?.TeamId}`, { state: { ...teamDetails } });
    }
                ...
                {waivers?.length > 0 && team?.TeamId === teamDetails?.TeamId ?
                    <Box sx={{
                        ...
                        <Typography sx={{
                            ...
                            <Link to={`/WaiverRequests/`}>{`${waivers?.length}`} pending</Link>
                        </Typography>
                    </Box>
                    : null}
                {team?.TeamId === teamDetails?.TeamId || user?.isAdmin || user?.isCommissioner ?
                    <Box sx={{
                        ...
                        {team?.TeamId === teamDetails?.TeamId ?
                            <Button sx={{ maxWidth: 300 }} variant="contained" to={`/TeamTrades`}>Trade Proposals</Button>
                            : null}
                    </Box>
                    : null}
            </Box>
