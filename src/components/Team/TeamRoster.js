...
function TeamRoster({ roster, setRoster, teamDetails, team, isEditable }) {
    ...
    const handleConfirmClick = async () => {
        ...
        setIsDeleting(true);
        const result = await deleteRosterPlayer(teamDetails.LeagueId, selectedPlayer.RosterPlayerId);
        setIsDeleting(false);
        if (result?.Message) {
            setDeleteMessage([result?.Message]);
        }
        else {
            const updatedPlayers = roster.filter((rosterPlayer) =>
    ...
    return (
        <>
            <ConfirmationDialog title={'Delete Player'} open={showDeleteConfirmation} setOpen={setShowDeleteConfirmation}
