...
function RosterPlayerAdd({ league, team }) {
    ...
    useEffect(() => {
        const fetchPlayer = async (teamId, playerId) => {
            ...
            setPlayer(response);
            const responsePlayerIds = await getRosterPlayersToDrop(league?.LeagueId, team?.TeamId, playerId);
            const responseRosters = await leaguePlayersLoader(league?.LeagueId, "All", "OnRosters", 1, 1000, 1, "All", "All", " ", "PositionId", "ASC");
            setRosterPlayers(mapToRosterList(responseRosters, teamId).find(rosterPlayer => rosterPlayer?.team.TeamId === teamId)?.Players.filter(rosterPlayer => responsePlayerIds.includes(rosterPlayer?.RosterPlayerId)));
            setIsLoading(false);
        }
