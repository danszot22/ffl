...
export default function DropRosterPlayers({
  ...
  const handleClickConfirm = async () => {
    ...
    const result = await updateRoster(
      ...
      selectedRosterPlayer.RosterPlayerId
    );
    setIsUpdating(false);
    if (result?.Message) {
        ...
        <DialogContent>
          <DialogContentText component="div" id="alert-dialog-description">
            <Typography component="div">
              {playerToAdd?.Name
                ? `Adding ${formatPlayerFullName(playerToAdd?.Name)} ${
                    playerToAdd?.Position?.PositionCode
                  }`
                : null}
            </Typography>
            <Typography component="div">
              {selectedRosterPlayer?.PlayerName
                ? `Dropping ${formatPlayerFullName(
                    selectedRosterPlayer?.PlayerName
                  )} ${selectedRosterPlayer?.PositionCode}`
                : null}
            </Typography>
