...
function ProposeTrade({ league, team }) {
  ...
  useEffect(() => {
    setProposedTeamRoster(
      rosters.find((rosterPlayer) => rosterPlayer?.team.TeamId === proposedTeam)
    );
  }, [proposedTeam, rosters]);
  ...
  useEffect(() => {
    setTeamRoster(
      rosters.find((rosterPlayer) => rosterPlayer?.team.TeamId === team?.TeamId)
    );
  }, [team?.TeamId, rosters]);
  ...
  const handlePlayerChange = (event) => {
    const player = teamRoster?.Players?.find(
    ...
    if (player) {
      if (event.target.checked) {
        ...
        const players = givingPlayers.filter(
          (rp) => rp.RosterPlayerId !== +event.target.name
        );
        setGivingPlayers(players);
      }
    }
    const proposedPlayer = proposedTeamRoster?.Players?.find(
      (rosterPlayer) => +event.target.name === rosterPlayer.RosterPlayerId
    );
    if (proposedPlayer) {
      if (event.target.checked) {
        ...
        const players = receivingPlayers.filter(
          (rp) => rp.RosterPlayerId !== +event.target.name
        );
  ...
  return (
    <Root title="Propose Trade">
      ...
      <Button variant="contained" sx={{ ml: 1 }} to={`/TeamTrades`}>
        Cancel
      </Button>
