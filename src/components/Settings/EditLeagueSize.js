...
function EditLeagueSize({ league }) {
  ...
  const handleChange = (event) => {
    if (!event.target.checked) {
      const updatedTeams = teamsToRemove?.filter(
        (teamId) => +event.target.Id === teamId
      );
    ...
    <Root title={"Change League Size"}>
      ...
      >
        ...
        <FormControl fullWidth>
          ...
          >
            {regularSchedules?.map((regularSchedule) => (
              <MenuItem
                key={regularSchedule.ScheduleId}
                value={regularSchedule.ScheduleId}
              >
        ...
        <FormControl fullWidth>
          <InputLabel id="playoff-schedule-select-label">
            Playoff Schedule
          </InputLabel>
          <Select
          ...
          >
            {playoffSchedules?.map((playoffSchedule) => (
              <MenuItem
                key={playoffSchedule.ScheduleId}
                value={playoffSchedule.ScheduleId}
              >
      ...
      {league?.NumberOfTeams > teamCount ? (
        <TableContainer sx={{ p: 2 }}>
          <TableHead>
            <TableRow>
              <TableCell colSpan={2}>Select Teams To Delete</TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {teams.map((team) => (
