...
export default function EditRosterSettings() {
  ...
  const handleChange = (id, value) => {
    if ((id === "QB" && TMQB) || (id === "PK" && TMPK)) id = "TM" + id;
    else if ((id === "SQB" && TMQB) || (id === "SPK" && TMPK)) id = "STM" + id;
    else if (id === "WR" && TE) id = "R";
    else if (id === "SWR" && TE) id = "R";
    const updatedSettings = {
      ...settings,
      [id]: value,
    };
  ...
  useEffect(() => {
    const editFields = Object.entries(settings).filter(
      (setting) =>
        !["SRBWR", "TMQB", "STMQB", "R", "SR", "TMPK", "STMPK"].includes(
          setting[0]
        )
    );
            ...
            >
              ...
              {index !== fields.length - 1 ? (
                <TextField
                  ...
                  value={
                    fields[index][0] === "QB"
                      ? settings.STMQB + fields[index + 1][1]
                      : fields[index][0] === "PK"
                      ? settings.STMPK + fields[index + 1][1]
                      : fields[index][0] === "WR"
                      ? settings.SR + fields[index + 1][1]
                      : fields[index + 1][1]
                  }
                  onChange={(event) =>
                    handleChange(event.target.id, +event.target.value)
                  ...
                  <FormControlLabel
                    control={
                      <Checkbox
                        name="TMQB"
                        checked={TMQB}
                        onChange={() => handleChangeQB(!TMQB)}
                      />
                    }
                    label={isBelowMedium ? "Team QB" : "Team Quarterbacks"}
                  />
                </FormGroup>
                  ...
                  <FormControlLabel
                    control={
                      <Checkbox
                        name="TMPK"
                        checked={TMPK}
                        onChange={() => handleChangePK(!TMPK)}
                      />
                    }
                    label={isBelowMedium ? "Team PK" : "Team Kickers"}
                  />
                </FormGroup>
                  ...
                  <FormControlLabel
                    control={
                      <Checkbox
                        name="TE"
                        checked={TE}
                        onChange={() => handleChangeTE(!TE)}
                      />
                    }
                    label={isBelowMedium ? "Incl TE" : "Include Tight Ends"}
                  />
                </FormGroup>
              ) : (
                <FormGroup sx={{ width: 100 }}></FormGroup>
              )}
            </Box>
          ) : null;
        })}
