...
export const playerNewsLoader = async (season, espnPlayerId, teamId) => {
    let news = [];
    try {
        const teamResponse = await getFromEspnApi(`http://sports.core.api.espn.com/v2/sports/football/leagues/nfl/teams/${teamId}/injuries?limit=1000`);
        ...
        const response = await getFromEspnApi(`http://sports.core.api.espn.com/v2/sports/football/leagues/nfl/seasons/${season}/athletes/${espnPlayerId}/injuries?limit=1000`);
    ...
    const result = Array.from(new Set(news.map(s => s.id)))
        .map(id => {
