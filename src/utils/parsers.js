...
export function mapSeasonRecords(data) {
    data.sort(
        (a, b) => (b.Wins) - (a.Wins) || (b.DivisionWins) - (a.DivisionWins)
    );
...
export function mapCategoryScoring(data) {
    ...
    data?.forEach((pointVersion) => {
        pointVersion.FantasyPoint.items?.forEach((fantasyPoint) => {
            ...
            const index = subCategoryMap[fantasyPoint.StatisticalCategory].teamStatistics.findIndex((teamStatistic) => {
            ...
            if (index < 0) {
                ...
                const previousStatistics = subCategoryMap[fantasyPoint.StatisticalCategory].teamStatistics[index];
                subCategoryMap[fantasyPoint.StatisticalCategory].teamStatistics[index] = {
                    ...previousStatistics,
                    PointTotal: fantasyPoint.StarterOrBench === 0 && !pointVersion.Projected ? fantasyPoint.PointTotal : previousStatistics.PointTotal,
                    BonusTotal: fantasyPoint.StarterOrBench === 0 && !pointVersion.Projected ? fantasyPoint.BonusTotal : previousStatistics.BonusTotal,
                    StatisticalTotal: fantasyPoint.StarterOrBench === 0 && !pointVersion.Projected ? fantasyPoint.StatisticalTotal : previousStatistics.StatisticalTotal,
                    ProjStatisticalTotal: fantasyPoint.StarterOrBench === 0 && pointVersion.Projected ? fantasyPoint.StatisticalTotal : previousStatistics.StatisticalTotal,
                    Bench: fantasyPoint.StarterOrBench === 1 ? mapPlayerCategoryTotal(previousStatistics.Bench, pointVersion.Projected, fantasyPoint.PlayerStatistic.items) : previousStatistics.Bench,
                    Starters: fantasyPoint.StarterOrBench === 0 ? mapPlayerCategoryTotal(previousStatistics.Starters, pointVersion.Projected, fantasyPoint.PlayerStatistic.items) : previousStatistics.Starters,
                };
            }
        });
    });
    Object.values(subCategoryMap).forEach((category) => {
        category.teamStatistics.sort(
            (a, b) => (b.PointTotal) - (a.PointTotal) || (b.BonusTotal) - (a.BonusTotal) || (b.StatisticalTotal) - (a.StatisticalTotal) || (b.ProjStatisticalTotal) - (a.ProjStatisticalTotal)
        );
...
export function mapSeasonCategoryScoring(data) {
        ...
        pointVersion.FantasyPoint.items?.forEach((fantasyPoint) => {
            ...
            if (index < 0) {
                subCategoryMap[fantasyPoint.StatisticalCategory]?.teamStatistics?.push({
                    ...
                    StatisticalTotal: fantasyPoint.StarterOrBench === 0 ? fantasyPoint.StatisticalTotal : 0,
                    Team: { ...fantasyPoint.Team },
                    TeamId: fantasyPoint.TeamId,
                    Starters: fantasyPoint.StarterOrBench === 0 ? mapPlayerSeasonCategoryTotal({}, fantasyPoint.PlayerStatistic.items, fantasyPoint.StatisticalCategory, fantasyPoint.TeamId) : {},
                });
            }
            else {
                const previousStatistics = subCategoryMap[fantasyPoint.StatisticalCategory].teamStatistics[index];
                subCategoryMap[fantasyPoint.StatisticalCategory].teamStatistics[index] = {
                    ...previousStatistics,
                    PointTotal: fantasyPoint.StarterOrBench === 0 ? fantasyPoint.PointTotal + previousStatistics.PointTotal : previousStatistics.PointTotal,
                    BonusTotal: fantasyPoint.StarterOrBench === 0 ? fantasyPoint.BonusTotal + previousStatistics.BonusTotal : previousStatistics.BonusTotal,
                    StatisticalTotal: fantasyPoint.StarterOrBench === 0 ? fantasyPoint.StatisticalTotal + previousStatistics.StatisticalTotal : previousStatistics.StatisticalTotal,
                    Starters: fantasyPoint.StarterOrBench === 0 ? mapPlayerSeasonCategoryTotal(previousStatistics.Starters, fantasyPoint.PlayerStatistic.items, fantasyPoint.StatisticalCategory, fantasyPoint.TeamId) : previousStatistics.Starters,
                };
            }
        });
    });
    Object.values(subCategoryMap).forEach((category) => {
        category.teamStatistics.sort(
            (a, b) => (b.PointTotal) - (a.PointTotal) || (b.BonusTotal) - (a.BonusTotal) || (b.StatisticalTotal) - (a.StatisticalTotal)
        );

        Object.values(category.teamStatistics).forEach((teamStatistic) => {
            teamStatistic.Starters = Object.values(teamStatistic.Starters).map((starter) => (
                starter
            ));
            teamStatistic.Starters.sort(
                (a, b) => (b.Total) - (a.Total)
...
const mapPlayerStatistic = (statistics, projected, statisticalCategoryId, newStatistic, index) => {
    ...
    const playerStatistic = {
        ...
        [(projected ? "Proj" + statisticalCategory[statisticalCategoryId] : statisticalCategory[statisticalCategoryId])]: newStatistic.Total,
    }

    const result = statistics.filter((statistic) => {
    ...
    data?.forEach((pointVersion) => {
        ...
        pointVersion.FantasyPoint.items?.forEach((fantasyPoint) => {
            ...
            if (pointVersion.Projected === true) {
                teamTotalMap[fantasyPoint.TeamId].projectedTotal += fantasyPoint.PointTotal + fantasyPoint.BonusTotal;
            } else {
                teamTotalMap[fantasyPoint.TeamId].total += fantasyPoint.PointTotal + fantasyPoint.BonusTotal;
            }

            fantasyPoint.PlayerStatistic.items.forEach((newStatistic) => {
                if (fantasyPoint.StarterOrBench === 1) {
                    const index = teamTotalMap[fantasyPoint.TeamId].Bench.findIndex((existingStatistic) => {
                        return existingStatistic.PlayerId === newStatistic.PlayerId;
                    });
                    teamTotalMap[fantasyPoint.TeamId].Bench = mapPlayerStatistic(teamTotalMap[fantasyPoint.TeamId].Bench, pointVersion.Projected, fantasyPoint.StatisticalCategory, newStatistic, index);
                }
                else {
                    const index = teamTotalMap[fantasyPoint.TeamId].Starters.findIndex((existingStatistic) => {
                        return existingStatistic.PlayerId === newStatistic.PlayerId;
                    });
                    teamTotalMap[fantasyPoint.TeamId].Starters = mapPlayerStatistic(teamTotalMap[fantasyPoint.TeamId].Starters, pointVersion.Projected, fantasyPoint.StatisticalCategory, newStatistic, index);
                }
            });
        });
    });
            ...
            teamLineup?.LineupPlayer?.items?.forEach((lp) => {
                if (lp.Starter) {
                    const player = teamScore.Starters.find((p) => {
                        return p.PlayerId === lp.RosterPlayer.PlayerId ||
                            (((lp.RosterPlayer.Player.Position?.PositionCode?.startsWith("TMQB") && p.Player.Position.PositionCode === "QB") ||
                                (lp.RosterPlayer.Player.Position?.PositionCode?.startsWith("TMPK") && p.Player.Position.PositionCode === "PK")) &&
                                lp.RosterPlayer.Player.NflTeam?.NflTeamId === p.Player.NflTeam?.NflTeamId);
                    });
                    if (!player) {
                        const nflGame = nflGames.find((nflGame) => {
                            return nflGame.HomeTeam?.NflTeamId === lp.RosterPlayer.Player?.NflTeam?.NflTeamId ||
                                nflGame.AwayTeam?.NflTeamId === lp.RosterPlayer.Player?.NflTeam?.NflTeamId;
                        });
                        const missingPlayer = {
...
export function mapFantasyGames(data) {
    ...
    data?.forEach((result) => {
        ...
        if (result.PointVersion.Projected === true) {
            fantasyGameMap[result.FantasyGame.FantasyGameId].ProjectedHomeTotal = result.HomePointTotal;
            fantasyGameMap[result.FantasyGame.FantasyGameId].ProjectedAwayTotal = result.AwayPointTotal;
        } else {
            fantasyGameMap[result.FantasyGame.FantasyGameId].HomeTotal = result.HomePointTotal;
            fantasyGameMap[result.FantasyGame.FantasyGameId].AwayTotal = result.AwayPointTotal;
        }
    });

    return Object.values(fantasyGameMap).map((game) => (
        game
    ));
}
...
export function mapToLineupList(data, scoringData, nflGames, team) {
    ...
    data?.forEach((teamLineup) => {
        ...
        teamLineup.LineupPlayer.items.forEach((teamLineupPlayer) => {
            const nflGame = nflGames.find((nflGame) => {
                return nflGame.HomeTeam?.NflTeamId === teamLineupPlayer.RosterPlayer.Player?.NflTeam?.NflTeamId ||
                    nflGame.AwayTeam?.NflTeamId === teamLineupPlayer.RosterPlayer.Player?.NflTeam?.NflTeamId;
            });
            if (teamLineupPlayer.Starter) {
                ...
                lineupMap[teamLineup.Team.TeamId].Starters = lineupMap[teamLineup.Team.TeamId].Starters.concat(player);
            }
            else {
                const playerScore = teamScore.Bench.find((benchPlayer) => {
                    return benchPlayer.PlayerId === teamLineupPlayer.RosterPlayer.PlayerId;
                });
                const player = {
                    NflGame: { ...mapNflGame(nflGame) },
                    ...teamLineupPlayer,
                ...
                lineupMap[teamLineup.Team.TeamId].Bench = lineupMap[teamLineup.Team.TeamId].Bench.concat(player);
            }
        });
    });
    ...
    lineupResult.sort(
        (a, b) => {
            return ((a.team.TeamId === team) ? 0 : 1) - ((b.team.TeamId === team) ? 0 : 1) || (new Date(b.SubmitDateTime) - new Date(a.SubmitDateTime));
        }
    );
...
function mapPositionGroup(positionCode) {
    return ["S", "CB", "LB", "DE", "DT"].includes(positionCode) ? "Defense" :
        ["WR", "TE"].includes(positionCode) ? "Receivers" :
            ["TMQB", "QB"].includes(positionCode) ? "Quarterbacks" :
                ["TMPK", "PK"].includes(positionCode) ? "Kickers" :
...
export function mapLineupToTeamLineup(lineup, nflGames) {
    ...
    lineup.LineupPlayer?.items?.forEach((lineupPlayer) => {
        ...
        const nflGame = nflGames?.find((nflGame) => {
            return nflGame.HomeTeam?.NflTeamId === lineupPlayer.RosterPlayer.Player.NflTeam.NflTeamId ||
                nflGame.AwayTeam?.NflTeamId === lineupPlayer.RosterPlayer.Player.NflTeam.NflTeamId;
        });
        const player = {
        ...
        lineupMap[lineup.Team.TeamId].Players = lineupMap[lineup.Team.TeamId].Players.concat(player);
    });
    const lineupResult = Object.values(lineupMap).map((lineup) => (
        lineup
    ));
...
export function mapToPlayerList(data, league) {
    const playerMap = data.map((player) => {
        const rosterPlayer = player.RosterPlayer?.items?.find((rosterPlayer) => {
            return rosterPlayer?.Team?.LeagueId === league && rosterPlayer.DeleteDate === null;
        });
        return {
            ...player,
...
export function mapToRosterList(data, team) {
    ...
    data?.forEach((rosterPlayer) => {
        ...
        rosterMap[rosterPlayer.TeamId].Players = rosterMap[rosterPlayer.TeamId].Players.concat(player);
    });
    const rosterResult = Object.values(rosterMap).map((roster) => (
        roster
    ));
    rosterResult.sort(
        (a, b) => ((a.team.TeamId === team) ? 0 : 1) - ((b.team.TeamId === team) ? 0 : 1)
    );
    return rosterResult;
}
...
function mapTeamGame(game, team) {
    ...
    game.FantasyGameResult?.items?.forEach((gameResult) => {
        if (gameResult.PointVersion.Projected) {
            mappedGame.ProjectedTotal = game.HomeTeam.TeamId === team ? gameResult.HomePointTotal : gameResult.AwayPointTotal;
            mappedGame.Opponent.ProjectedTotal = game.HomeTeam.TeamId === team ? gameResult.AwayPointTotal : gameResult.HomePointTotal;
        }
        else {
            mappedGame.Complete = gameResult.PointVersion.Complete;
            mappedGame.PointTotal = game.HomeTeam.TeamId === team ? gameResult.HomePointTotal : gameResult.AwayPointTotal;
            mappedGame.Opponent.PointTotal = game.HomeTeam.TeamId === team ? gameResult.AwayPointTotal : gameResult.HomePointTotal;
            mappedGame.Result = game.HomeTeam.TeamId === team && gameResult.AwayPointTotal > gameResult.HomePointTotal ? "L" :
                game.HomeTeam.TeamId === team && gameResult.AwayPointTotal < gameResult.HomePointTotal ? "W" :
                    game.HomeTeam.TeamId === team && gameResult.AwayPointTotal === gameResult.HomePointTotal ? "T" :
                        game.AwayTeam.TeamId === team && gameResult.AwayPointTotal > gameResult.HomePointTotal ? "W" :
                            game.AwayTeam.TeamId === team && gameResult.AwayPointTotal < gameResult.HomePointTotal ? "L" :
                                game.AwayTeam.TeamId === team && gameResult.AwayPointTotal === gameResult.HomePointTotal ? "T" : " ";
        }
    });
...
export function mapTeamFinances(teamData, prizeData, transactionData) {
    ...
    const transactionTotals = transactionData?.reduce((acc, transaction) => {
        acc[transaction.RosterPlayerAdded.Team.TeamId] = (acc[transaction.RosterPlayerAdded.Team.TeamId] || 0) + transaction.TransactionFee;
        return acc;
    }, {});
    ...
    const finances = teamData.map((team) => {
        const mappedTeam = {
            ...
            TransactionFees: (transactionTotals && transactionTotals[team.TeamId]) || 0,
            Balance: (team.EntryFee + ((transactionTotals && transactionTotals[team.TeamId]) || 0) - team.FeesPaid) - ((prizeTotals[team.TeamId] || 0) - team.WinningsPaid)
        };
        return mappedTeam;
    });
...
export function mapPlayerDetails(data, gameData) {
    ...
    data?.PlayerStatistic.items.forEach((playerStatistic) => {
        ...
        if (!playerStatistic.PlayerStatisticVersion.Projected) {
            gameMap[playerStatistic.PlayerStatisticVersion.NflGame?.NflGameId] = {
                ...gameMap[playerStatistic.PlayerStatisticVersion.NflGame?.NflGameId],
                [statisticalCategory[playerStatistic.StatisticalCategory]]: playerStatistic.Total,
            }
        }
    });
